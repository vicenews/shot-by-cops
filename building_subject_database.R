...
#Remove suicides.
r <- subset(r, !grepl("self-inflicted|self inflicted", FullNarrative))
r <- subset(r, !grepl("self-inflicted|self inflicted", Notes))
r <- subset(r, !grepl("https://www.lvmpd.com/en-us/InternalOversightConstitutionalPolicing/Documents/Non-Fatal%20OIS/FITReport_120316-4534_NonFatal_FINAL.pdf", FullNarrative))
r <- subset(r, !grepl("Full narrative via news reports at: http://www.elpasotimes.com/story/news/crime/2015/09/19/autopsy-charles-bertram-death-suicide/72440626/", FullNarrative))
r <- subset(r, !grepl("Suspect then committed suicide.", FullNarrative))
...
r <- subset(r, !city%in%c(
"Colorado Springs","Oklahoma City",
"Wichita","Oakland","Raleigh", "Virginia",
"Long Beach"))

#and remove some rogue departments
r <- subset(r, !Department%in%c("Fairfield County Sheriff's Office",
"Franklin County Adult probation officer",
"Franklin County Sheriff's Office",
"Ohio State Highway Patrol", "Westerville Police",
"Denver Sheriffs Department","Unknown"))
...
SubjectRace=NA,
SubjectGender=NA, SubjectAge=NA,
...
b=NA, l=NA, w=NA, a=NA, tr=NA,
AvgAge=NA, VA=NA, F=NA, nshots=NA,
sit=NA
)
...
x$NumberOfSubjects[i]==length(unlist(strsplit(
as.character(x$SubjectArmed[i]), split=";"))))
{z <- append(z, unlist(strsplit(
as.character(x$SubjectArmed[i]), 
split=";")))}
if(!x$NumberOfSubjects[i]==length(unlist(strsplit(
as.character(x$SubjectArmed[i]), split=";")))&
...
grepl("vehicle|rock|taser|blunt|pipe|misc weapon|screwdriver|crowbar|shears|cane|pole|rod|scissors", tolower(vic$Notes))
...
grepl("vehicle|rock|taser|blunt|pipe|misc weapon|screwdriver|crowbar|shears|cane|pole|rod|scissors", tolower(r$Notes))
