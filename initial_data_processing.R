...
##data input
v <- list()
for (i in 1:length(grep("csv", list.files(), value = T)))
{
  ...
  v[[i]]$city <- substr(unlist(lapply(strsplit(
    grep("csv",
         list.files(), value = T), "_"
  ), "[[", 1))[i], 9, 100)
}
...
dat$Date[grepl("T", dat$Date)] <-
  unlist(lapply(strsplit(as.character(dat$Date[grepl("T", dat$Date)]),
                         split = "T"), "[[", 1))
...
dat$year[dat$city == "Portland"] <-
  year(mdy(unlist(lapply(
    strsplit(subset(dat, city == "Portland")$Date,
             split = " "), "[[", 1
  ))))
dat$year[dat$city == "CharlotteMecklenburg"] <-
  (unlist(lapply(
    strsplit(subset(dat, city == "CharlotteMecklenburg")$Date,
             split = "-"), "[[", 1
  )))
...
dat$fdate[dat$city == "CharlotteMecklenburg"] <-
  paste(
    subset(dat, city == "CharlotteMecklenburg")$month,
    paste("1",
...
dat$Notes[dat$Fatal %in% c("Suicide")] <-
  paste(dat$Notes, "Suicide", sep = ". ")[dat$Fatal %in% c("Suicide")]
dat$Fatal[dat$Fatal == "Suicide"] <- "N"
dat$Fatal[dat$Fatal == "Continued"] <- "U"
dat$Notes[dat$Fatal %in% c("No hit")] <-
  paste(dat$Notes, "No hit", sep = ". ")[dat$Fatal %in% c("No hit")]
dat$Notes[dat$Fatal %in% c("Miss")] <-
  paste(dat$Notes, "No hit", sep = ". ")[dat$Fatal %in% c("Miss")]
...
dat$SubjectRace[dat$SubjectRace == "Black"] <- "B"
dat$SubjectRace[dat$SubjectRace == "Black or African American"] <-
  "B"
...
dat$SubjectRace[dat$SubjectRace == "Asian or PaciFc islander"] <-
  "A"
...
dat$SubjectGender <-
  gsub("UNKNOWN", "U", toupper(dat$SubjectGender))
...
v <- c(
  ...
  "N(Knife)",
  "N (Knife)",
  "Fake gun i.e. black phone",
  "Crowbar",
  "N(vehicle)",
  "N(motorized vehicle)",
  "N(knife/cutting instrument)",
  "N(knife)",
  "Handgun Replica",
  "No Weapon",
  "Rifle & Handgun",
...
dat$SubjectArmed[dat$SubjectArmed %in% c(
  "UNK",
  "Unclear",
  "Pending",
  "Other",
...
dat$AvgAge <- unlist(lapply(lapply(strsplit(dat$SubjectAge, split = ";"), function(x)
{
  as.numeric(as.character(x))
}),
mean, na.rm = T))
...
#number of shots
dat$nshots <- unlist(lapply(str_extract_all(dat$NumberOfShots, "[0-9]*"), function(x)
{
...
dat$sit[grepl("suspect|wanted|warrant", tolower(dat$NatureOfStop))] <-
  "suspect"
dat$sit[grepl("car|vehicle|speeding|traffic",
              tolower(dat$NatureOfStop))] <- "traffic"
dat$sit[grepl("stolen vehicle|stolen car", tolower(dat$NatureOfStop))] <-
  "stolenvehicle"
