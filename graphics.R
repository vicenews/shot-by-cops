...
subject_db <-
  subject_db %>% mutate_if(is.factor, funs(factor(replace(., . == "", NA))))
...
subject_counts <-
  subject_db %>% group_by(Fatal) %>% summarize(total = n())
...
subjects_overtime <-
  ggplot(subject_counts_byyear, aes(x = year, y = total)) +
...
subject_counts_race <-
  subject_db_race %>% group_by(SubjectRace) %>% summarize(total_race = n())
...
city_pops_race_wide <-
  gather(city_pops_race_totals, race, total_pop)

#join subject race with city pop by race
subject_race_pop <-
  left_join(city_pops_race_wide,
            subject_counts_race,
            by = c("race" = "SubjectRace"))

#calculate rate per race per 100K
subject_race_pop_rate <-
  subject_race_pop %>% mutate(rate_100k = ((total_race / total_pop) / 7) *
                                100000)

#set levels
subject_race_pop_rate$race <-
  factor(subject_race_pop_rate$race, levels = subject_race_pop_rate$race[order(subject_race_pop_rate$rate_100k)])

#OIS rates by rate per year
ratesByRaceChart <-
  ggplot(subject_race_pop_rate, aes(x = race, y = rate_100k)) +
...
blackArmed_pct <-
  armedRace_counts %>% filter(SubjectRace == 'B') %>% mutate(pct = total /
                                                               (sum(total)))
whiteArmed_pct <-
  armedRace_counts %>% filter(SubjectRace == 'W') %>% mutate(pct = total /
                                                               (sum(total)))
asianArmed_pct <-
  armedRace_counts %>% filter(SubjectRace == 'A') %>% mutate(pct = total /
                                                               (sum(total)))
latinoArmed_pct <-
  armedRace_counts %>% filter(SubjectRace == 'L') %>% mutate(pct = total /
                                                               (sum(total)))
otherArmed_pct <-
  armedRace_counts %>% filter(SubjectRace == 'O') %>% mutate(pct = total /
                                                               (sum(total)))
unknownArmed_pct <-
  armedRace_counts %>% filter(SubjectRace == 'U') %>% mutate(pct = total /
                                                               (sum(total)))
...
#set levels
all_race_pcts$weapon <-
  factor(all_race_pcts$weapon,
         levels = c("unarmed", "U", "replica", "other", "knife", "gun"))

raceArmedChart <-
  ggplot(all_race_pcts, aes(x = "", y = pct, fill = weapon)) +
...
all_race_pcts2 <-
  all_race_pcts %>% filter(!(SubjectRace == 'U' |
                               SubjectRace == 'A' | SubjectRace == 'O'))
...
raceArmedChart2 <-
  ggplot(all_race_pcts2, aes(x = "", y = pct, fill = weapon)) +
  geom_bar(stat = "identity", width = 0.5) +
  scale_fill_manual(values = c(
    "#fdcc8a",
    "#cdcdcd",
...
incident_db_clean$SubjectArmed[is.na(incident_db_clean$SubjectArmed)] <-
  "U"

#remove any whitespace in VictmArmed
incident_db_clean <-
  incident_db_clean %>% mutate(SubjectArmed = gsub(' ', '', SubjectArmed))

#get total incident counts with weapon
incident_counts <-
  incident_db_clean %>% group_by(weapon) %>% summarize(total = n())

#get percentage of incidents subject is armed
incident_counts_pct <-
  incident_counts %>% mutate(pct = total / (sum(total)))

#set levels
incident_counts_pct$weapon <-
  factor(
    incident_counts_pct$weapon,
    levels = c("unarmed", "U", "replica", "other", "knife", "gun")
  )
...
incidentsArmedChart <-
  ggplot(incident_counts_pct, aes(x = "", y = pct, fill = weapon)) +
  geom_bar(stat = "identity", width = 0.5) +
  scale_fill_manual(values = c(
    "#fdcc8a",
    "#cdcdcd",
...
city_counts_byyear <-
  subject_db_fullyears %>% group_by(city, year) %>% summarize(total = n())

#remove San Diego 2016 incomplete data
city_counts_byyear <-
  city_counts_byyear %>% filter(!(city == "SanDiego" & year == "2016"))

#join with population data
city_count_pop <-
  left_join(city_pops, city_counts_byyear, by = c("city_name" = "city"))
city_count_pop <-
  city_count_pop %>% select(department, city_pop_2013, year, total) %>% filter(department !=
                                                                                 'Detroit (MI) Police')

#calculate rates per 100k in each City
city_rates <-
  city_count_pop %>% mutate(rate_100k = (total / city_pop_2013) * 100000)
city_rates <- city_rates %>% select(department, year:rate_100k)

#make data wide format
city_wide <-
  spread(city_rates, year, rate_100k) %>% select(-total) %>%
  group_by(department) %>%
...
city_rate_avgs <-
  city_wide %>% mutate(
    rateavg1013 = ifelse(
      department == 'Washington (DC) Metropolitan Police' |
        department == 'Louisville (KY) Metro Police ',
      (`2011` + `2012` + `2013`) / 3,
  ...
  mutate(
    rateavg1416 = ifelse(
      department == 'San Diego (CA) Police ' |
        department == 'Atlanta (GA) Police',
      (`2014` + `2015`) / 2,
      (`2014` + `2015` + `2016`) / 3
    )
  ) %>%
...
city_top10decreases$department <-
  factor(city_top10decreases$department, levels = city_top10decreases$department[order(city_top10decreases$ratechg)])

#select just rate columns
city_rates_only <-
  city_top10decreases %>% select(department, `2010`:`2016`)
...
chart_cityrates <- ggplot() +
  geom_line(data = city_rates_long, aes(
    x = year,
    y = rate,
    group = department,
    color = factor(department)
  )) +
...
#select just rate columns
city_rates_all <-
  city_rate_avgs %>% select(department, `2010`:`2016`)

#make long, clean up years
city_rates_long_all <-
  gather(city_rates_all, year, rate,-department)

#chart of all cities
chart_cityrates_all <- ggplot() +
  geom_line(data = city_rates_long_all, aes(
    x = year,
    y = rate,
    group = department,
    color = "#ff011f"
...
#join with pop data
detroit_ois <-
  left_join(city_pops, detroit_ois, by = c("city_name" = "city"))
detroit_yearly <-
  detroit_ois %>% group_by(year, city_pop_2013) %>% summarize(total = n()) %>% mutate(rate_100k =
                                                                                        (total / city_pop_2013) * 100000)

#chart detroit
chart_detroit <-
  ggplot(detroit_yearly,
         aes(
           x = year,
           y = rate_100k,
           group = 1,
